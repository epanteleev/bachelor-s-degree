...
\begin{frame}{Язык Java}
	\begin{enumerate}
		...
		\item Java упрощает написание кода из-за встроенной возможности сборки мусора и
		синхронизации потоков. 
	\end{enumerate}
\end{frame}

\begin{frame}{Модуль обработки данных}
	Как можно организовать работу с датчиками.
	\begin{enumerate}
		\item Создать поток на каждый датчик. 
			\begin{itemize}
				\item Плюсы: простота реализации.
				\item Минусы: высокое потребление системных ресурсов: памяти и процессорного времени.
			\end{itemize}
		\item Линейный опрос датчиков на доступность данных. 
			\begin{itemize}
				\item Плюсы: меньшее потребление ресурсов системы.
				\item Минусы: тяжелее реализовать.
			\end{itemize}
	\end{enumerate}
	Существует альтернативный способ организации -- \textbf{сопрограммы}. 
\end{frame}
...
\begin{frame}{Ключевые отличия от потоков ОС}
	\begin{itemize}
		\item Переключение контекста сопрограммы требует меньше накладных расходов, чем потока. Скорость переключения сопрограмм в 5--7 раз больше, чем у потоков.
